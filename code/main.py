...
def main(args: Args):
    ...
    try:
        chunks_info = concurrent_model.start(producer, consumer ,args)
        print("finish")
...
def setup_producer_consumer(args: Args) -> tuple[Producer, Consumer]:
    ...
    if args.filter_mode == FilterMode.AhoCorasick:
        text_filter: TextFilter = AhoCorasickFilter(bad_words , args)
    else:
        text_filter: TextFilter = RegexFilter(bad_words , args)
